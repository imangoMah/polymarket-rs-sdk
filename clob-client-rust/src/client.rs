...
/// TypeScript parity payload for cancellation: `{ orderID: string }`.
/// 使用 serde rename 保持 JSON 键与 TS 保持一致。
#[derive(Clone, Debug, serde::Serialize)]
pub struct OrderPayloadParity {
    #[serde(rename = "orderID")]
    pub order_id: String,
}

impl OrderPayloadParity {
...
/// TypeScript `OrderMarketCancelParams` parity: `{ market?: string, asset_id?: string }`
#[derive(Clone, Debug, serde::Serialize)]
...
impl ClobClient {
    ...
    pub async fn cancelOrder(&self, payload: OrderPayloadParity) -> Result<Value, ClobError> {
        self.cancel_order_payload_raw(payload).await
    }
    ...
    #[allow(non_snake_case)]
    pub async fn cancelMarketOrders(
        &self,
        payload: OrderMarketCancelParams,
    ) -> Result<Vec<Order>, ClobError> {
        self.cancel_market_orders(payload).await
    }
    ...
    pub async fn revokeBuilderApiKey(&self) -> Result<(), ClobError> {
        self.revoke_builder_api_key().await
    }
    ...
    pub async fn getBuilderTrades(
        &self,
        params: Option<std::collections::HashMap<String, String>>,
    ) -> Result<Vec<crate::types::BuilderTrade>, ClobError> {
    ...
    pub async fn cancel_market_orders(
        ...
        let signer_ref: &EthersSigner = signer_arc.as_ref();
        let body_str =
            serde_json::to_string(&payload).map_err(|e| ClobError::Other(e.to_string()))?;
        let ts = if self.use_server_time {
            Some(self.get_server_time().await?)
        } else {
            None
        ...
        // Serialize payload to generic Value for request data
        let body_val =
            serde_json::to_value(&payload).map_err(|e| ClobError::Other(e.to_string()))?;
        let raw: MaybeVec<Order> = crate::http_helpers::del_typed(
            &endpoint,
            Some(RequestOptions::<Value> {
                headers: Some(headers),
    ...
    pub async fn cancel_order_payload(
        &self,
        ...
        if let Some(b) = &self.builder_signer {
            let b_payload = b
                .create_builder_header_payload("DELETE", CANCEL_ORDER, Some(&body_str), None)
                .map_err(|e| ClobError::Other(format!("builder header error: {}", e)))?;
            headers = crate::headers::inject_builder_headers(headers, &b_payload);
        }
        let endpoint = format!("{}{}", self.host, CANCEL_ORDER);
        // Serialize body as generic Value for RequestOptions
        let body_val =
            serde_json::to_value(&payload).map_err(|e| ClobError::Other(e.to_string()))?;
    ...
    pub async fn cancel_order_payload_raw(
        &self,
        payload: OrderPayloadParity,
    ) -> Result<Value, ClobError> {
        if self.creds.is_none() {
        ...
        let raw: MaybeItem<Value> = crate::http_helpers::del_typed(&endpoint, Some(opts)).await?;
        Ok(raw.into_item())
    }
    ...
    pub async fn cancel_orders(&self, order_ids: Vec<String>) -> Result<Vec<Order>, ClobError> {
        ...
        if let Some(b) = &self.builder_signer {
            let b_payload = b
                .create_builder_header_payload("DELETE", CANCEL_ORDERS, Some(&body_str), None)
                .map_err(|e| ClobError::Other(format!("builder header error: {}", e)))?;
            headers = crate::headers::inject_builder_headers(headers, &b_payload);
        }
        ...
        let opts = RequestOptions {
            headers: Some(headers),
            data: Some(order_ids.clone()),
            params: None,
        };
        let raw: MaybeVec<Order> = crate::http_helpers::del_typed(&endpoint, Some(opts)).await?;
        Ok(raw.into_vec())
    }
    ...
    pub async fn get_builder_api_keys(&self) -> Result<Vec<crate::types::ApiKeyCreds>, ClobError> {
        ...
        let resp: Vec<crate::types::ApiKeyCreds> = crate::http_helpers::get_typed(
            &endpoint,
            Some(RequestOptions::<Value> {
                headers: Some(headers),
                data: None,
    ...
    pub async fn revoke_builder_api_key(&self) -> Result<(), ClobError> {
        if self.builder_signer.is_none() {
            return Err(ClobError::Other("Builder signer required".to_string()));
        }
        let b_signer = self.builder_signer.as_ref().unwrap();
        let headers_map = b_signer
            .create_builder_header_payload("DELETE", REVOKE_BUILDER_API_KEY, None, None)
            .map_err(|e| ClobError::Other(format!("builder header error: {}", e)))?;
    ...
    pub async fn get_builder_trades(
        ...
        let b_signer = self.builder_signer.as_ref().unwrap();
        let headers_map = b_signer
            .create_builder_header_payload("GET", GET_BUILDER_TRADES, None, None)
            .map_err(|e| ClobError::Other(format!("builder header error: {}", e)))?;

        // Convert HashMap<String, String> to Headers
        let mut headers = crate::headers::Headers::new();
        for (k, v) in headers_map {
            headers.insert(k, v);
        }
        ...
        let trades: Vec<crate::types::BuilderTrade> =
            serde_json::from_value(arr).map_err(|e| ClobError::Other(e.to_string()))?;
        Ok(trades)
    }
