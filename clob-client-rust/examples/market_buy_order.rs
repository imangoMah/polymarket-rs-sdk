...
// Example: build a market BUY order
// Run: cargo run --example market_buy_order -- <EXCHANGE_ADDRESS> <TOKEN_ID>
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let price = compute_market_price_from_book(&book, Side::BUY, 5.0, OrderType::FOK)?;

    let mbo = UserMarketOrder {
        token_id,
        price,
        amount: 5.0,
        side: Side::BUY,
    ...
    let signed = ob.build_market_order(&exchange, &mbo, "0.001").await?;
    println!(
        "Market BUY signed order: salt={} maker_amount={} taker_amount={} signature={}",
