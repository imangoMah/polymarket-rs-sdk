// 市价买单示例 (Market Buy Order with Builder)
// 演示如何创建并提交市价买单,包括使用 Builder 配置进行订单归属
//
// 环境变量:
//   必需:
//     PK - 私钥
...
//
// 参数:
//   cargo run --example market_buy_demo -- <TOKEN_ID> [AMOUNT]
//   例如: cargo run --example market_buy_demo -- 123456 5.0
//
// 注意: 市价单会立即以当前最佳价格成交
//
// 运行: cargo run --example market_buy_demo
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let order_book = client.getOrderBook(&token_id).await?; // 外部获取
    ...
    let user_market_order = UserMarketOrder {
        ...
        price, // 由外部 helper 计算后传入 (必填)
        amount,
        side: Side::BUY,
        fee_rate_bps: 0.0,          // 必填: 外部提供费率 (示例用 0bps)
        nonce: None,                // 自动生成
        taker: None,                // 任何人可成交
